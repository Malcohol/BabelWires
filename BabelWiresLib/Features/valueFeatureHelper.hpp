...
namespace babelwires {
    ...
    class ValueFeatureHelper {
      public:
        ...
        static std::tuple<const Feature*, unsigned int, Range<unsigned int>, unsigned int> getInfoFromArrayFeature(const Feature* f);

        /// Confirm that the feature is a record and return the state of the optionals.
        static std::tuple<const Feature*, std::map<ShortId, bool>> getInfoFromRecordWithOptionalsFeature(const Feature* f);
        ...
        static std::tuple<const Feature*, bool, std::vector<ShortId>> getInfoFromRecordWithVariantsFeature(const Feature* f, std::optional<ShortId> tagId = {});
    };
}
