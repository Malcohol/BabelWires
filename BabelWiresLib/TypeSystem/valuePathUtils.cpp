...
namespace {
    ...
    std::optional<std::tuple<const babelwires::Type&, const babelwires::ValueHolder&>> tryFollow(const babelwires::TypeSystem& typeSystem, const babelwires::Type& type, const babelwires::ValueHolder& valueHolder, const babelwires::Path& p, int& index) {
    ...
    std::tuple<const babelwires::Type&, babelwires::ValueHolder&> followNonConst(const babelwires::TypeSystem& typeSystem, const babelwires::Type& type, babelwires::ValueHolder& valueHolder, const babelwires::Path& p, int& index) {
        if (index < p.getNumSteps()) {
...
std::optional<std::tuple<const babelwires::Type&, const babelwires::ValueHolder&>> babelwires::tryFollowPath(const TypeSystem& typeSystem, const Type& type, const Path& path, const ValueHolder& start) {
    int index = 0;
    return tryFollow(typeSystem, type, start, path, index);
}


std::tuple<const babelwires::Type&, babelwires::ValueHolder&> babelwires::followPathNonConst(const TypeSystem& typeSystem, const Type& type, const Path& path, ValueHolder& start) {
    int index = 0;
    try {
