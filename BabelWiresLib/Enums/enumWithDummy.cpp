...
babelwires::Enum::EnumValues babelwires::EnumWithDummy::ensureDummyValue(const EnumValues& srcValues) {
    if (srcValues.size() > 0) {
        if (srcValues[srcValues.size() - 1] == getDummyValue()) {
            return srcValues;
        }
        assert((std::find(srcValues.begin(), srcValues.end(), getDummyValue()) == srcValues.end()) && "Found dummy value not at end of EnumValue vector.");
    }
    babelwires::Enum::EnumValues newValues = srcValues;
    newValues.emplace_back(getDummyValue());
    return newValues;
}
