...
const babelwires::RecordType::Field&
babelwires::RecordType::getField(ShortId fieldId) const {
    for (const auto& f : m_fields) {
...
void babelwires::RecordType::activateField(const TypeSystem& typeSystem, ValueHolder& value,
...
const babelwires::RecordType::Field&
babelwires::RecordType::getFieldFromChildIndex(const ValueHolder& compoundValue,
...
int babelwires::RecordType::getChildIndexFromStep(const ValueHolder& compoundValue,
...
babelwires::SubtypeOrder babelwires::RecordType::compareSubtypeHelper(const TypeSystem& typeSystem,
    ...
    while ((thisIt < thisFields.end()) && (otherIt < otherFields.end())) {
        if (thisIt->m_identifier == otherIt->m_identifier) {
            ...
            // A record with an additional _optional_ field can actually have the same set of valid values as one without
            // it, since we use "duck typing".
            ...
            if ((otherIt->m_optionality == Optionality::alwaysActive) && updateAndCheckUnrelated(SubtypeOrder::IsSupertype)) {
                return SubtypeOrder::IsUnrelated;
            }
            ++otherIt;
        }
