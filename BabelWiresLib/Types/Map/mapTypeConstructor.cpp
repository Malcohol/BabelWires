...
babelwires::ResultT<babelwires::MapEntryData::Kind>
babelwires::MapTypeConstructor::extractValueArguments(const TypeSystem& typeSystem,
    ...
    if (valueArguments.size() > 1) {
        return Error() << "MapTypeConstructor expects at most 1 value argument but got "
                       << valueArguments.size();
    ...
    if (const EnumValue* enumValue = valueArguments[0]->tryAs<EnumValue>()) {
        ...
        return Error() << "Value argument 0 given to MapTypeConstructor was not a MapEntryFallbackKind";
    }
}
...
babelwires::MapTypeConstructor::constructType(const TypeSystem& typeSystem, TypeExp newTypeExp,
    ...
    if (arguments.getTypeArguments().size() != 2) {
        return Error() << "MapTypeConstructor expects 2 type arguments but got "
                       << arguments.getTypeArguments().size();
    }
    ASSIGN_OR_ERROR(auto kind, extractValueArguments(typeSystem, arguments.getValueArguments()));
