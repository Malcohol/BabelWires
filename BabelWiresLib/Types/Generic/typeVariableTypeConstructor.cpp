...
babelwires::ResultT<babelwires::TypeVariableData>
babelwires::TypeVariableTypeConstructor::extractValueArguments(const std::vector<ValueHolder>& valueArguments) {
    ...
    if (valueArguments.size() != 2) {
        return Error() << "TypeVariableTypeConstructor expects 2 value arguments but got "
                       << valueArguments.size();
    }
    ...
    return TypeVariableData{static_cast<unsigned int>(args[0]), static_cast<unsigned int>(args[1])};
}
...
babelwires::TypeVariableTypeConstructor::constructType(const TypeSystem& typeSystem, TypeExp newTypeExp,
    ...
    DO_OR_ERROR(extractValueArguments(arguments.getValueArguments()));

    assert(resolvedTypeArguments.size() == arguments.getTypeArguments().size());
    if (arguments.getTypeArguments().size() > 1) {
        return Error() << "TypeVariableTypeConstructor expects 0 or 1 type arguments but got "
