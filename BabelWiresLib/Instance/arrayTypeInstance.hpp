...
namespace babelwires {
    ...
    class InstanceImpl<VALUE_FEATURE, ARRAY_TYPE> : public InstanceCommonBase<VALUE_FEATURE, ARRAY_TYPE> {
      public:
        ...
        ConstInstance<typename ARRAY_TYPE::EntryTypeForInstance> getEntry(unsigned int index) const {
            return InstanceUtils::getChild(this->m_valueFeature, index);
        }
        template <typename VALUE_FEATURE_M = VALUE_FEATURE>
        std::enable_if_t<!std::is_const_v<VALUE_FEATURE_M>,
                         Instance<typename ARRAY_TYPE::EntryTypeForInstance>>
