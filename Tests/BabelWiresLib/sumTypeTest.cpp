...
TEST(SumTypeTest, compareSubtype) {
    ...
    const babelwires::TypePtr textType = testEnvironment.m_typeSystem.getRegisteredType<babelwires::TextType>();
    ...
    EXPECT_EQ(testEnvironment.m_typeSystem.compareSubtype(*textType, *sumType), babelwires::SubtypeOrder::IsDisjoint);
    EXPECT_EQ(testEnvironment.m_typeSystem.compareSubtype(*sumType, *textType), babelwires::SubtypeOrder::IsDisjoint);
}
...
TEST(SumTypeTest, compareSubtype2) {
    ...
    const babelwires::TypeExp SExp = babelwires::TextType::getThisIdentifier();
...
TEST(SumTypeTest, subTypeAssociativity) {
    ...
    const babelwires::TypeExp S = babelwires::TextType::getThisIdentifier();
