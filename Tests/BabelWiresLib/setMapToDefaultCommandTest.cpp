...
TEST(SetMapToDefaultCommandTest, executeAndUndo) {
    ...
    babelwires::SetMapToDefaultCommand command("Set to default");

    EXPECT_TRUE(command.initialize(mapProject));
    command.execute(mapProject);

    EXPECT_EQ(mapProject.getNumMapEntries(), 1);
    EXPECT_EQ(mapProject.getMapEntry(0).getData().getKind(), babelwires::MapEntryData::Kind::AllToOne);
...
TEST(SetMapToDefaultCommandTest, invalidOldMap) {
    babelwires::IdentifierRegistryScope identifierRegistry;
    testUtils::TestEnvironment environment;
    environment.m_typeSystem.addEntry(std::make_unique<testUtils::TestType>());
