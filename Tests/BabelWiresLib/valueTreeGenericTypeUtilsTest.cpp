...
TEST(ValueTreeGenericTypeUtilsTest, containsUnassignedTypeVariable) {
    ...
    auto checkForVariable = [&](const babelwires::Path& pathToVariable) {
        const babelwires::ValueTreeNode& variable = babelwires::assertFollowPath(pathToVariable, valueTree);
...

    {
        ...
        nestedData.m_typeAssignments[0] = babelwires::DefaultTextType::getThisIdentifier();
...
TEST(ValueTreeGenericTypeUtilsTest, getTypeVariableAssignments) {
    ...
    babelwires::ValueTreeNode& targetNode =
        babelwires::assertFollowPath(testDomain::TestGenericType::getPathToNestedWrappedType(), targetTree);
    ...
    }
    {
        ...
        ASSERT_NE(it, assignments->end());
        EXPECT_EQ(it->second, babelwires::DefaultTextType::getThisIdentifier());
    }
}
