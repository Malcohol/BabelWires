...
TEST(SelectOptionalsModifierDataTest, apply) {
    ...
    babelwires::ValueTreeRoot valueFeature(testEnvironment.m_projectContext.get<babelwires::TypeSystem>(),
                                                testEnvironment.m_projectContext.get<babelwires::TypeSystem>().getRegisteredType<testDomain::TestComplexRecordType>());
