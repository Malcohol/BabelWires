...
TEST(ProjectBundleTest, filePathResolution) {
    ...
    for (const auto& scenario : scenarios) {
        ...
        {
            auto elementData = projectData.m_nodes[0].get()->tryAs<babelwires::SourceFileNodeData>();
            ASSERT_NE(elementData, nullptr);
            EXPECT_EQ(elementData->m_filePath, scenario.m_expectedResolvedPath);
        }
        {
            auto elementData = projectData.m_nodes[1].get()->tryAs<babelwires::TargetFileNodeData>();
            ASSERT_NE(elementData, nullptr);
            EXPECT_EQ(elementData->m_filePath, scenario.m_expectedResolvedPath);
        }
    }
...
"41000000-1111-2222-3333-888888888888", babelwires::IdentifierRegistry::Authority::isAuthoritative);
    babelwires::IdentifierRegistry::write()->addLongIdWithMetadata(testDomain::TestSourceFileFormat::getThisIdentifier(),
                                                             "Other test source factory",
"41000000-1111-2222-3333-999999999999", babelwires::IdentifierRegistry::Authority::isAuthoritative);
    babelwires::IdentifierRegistry::write()->addLongIdWithMetadata(testDomain::TestTargetFileFormat::getThisIdentifier(),
...
"41000000-1111-2222-3333-aaaaaaaaaaaa", babelwires::IdentifierRegistry::Authority::isAuthoritative);
    ...
    testUtils::TestEnvironment testEnvironment;
    testEnvironment.m_projectContext.get<babelwires::TargetFileFormatRegistry>().getEntryByIdentifier(projectData.m_elements[0]->m_factoryIdentifier);
    testEnvironment.m_projectContext.get<babelwires::ProcessorFactoryRegistry>().getEntryByIdentifier(projectData.m_elements[1]->m_factoryIdentifier);
    testEnvironment.m_projectContext.get<babelwires::SourceFileFormatRegistry>().getEntryByIdentifier(projectData.m_elements[2]->m_factoryIdentifier);
